...
export const useRouteFn = () => {
	 ...
	 * @param api 接口
	 */
	const initDynamicRoutes = async (api?: BackendApi) => {
		...
		if (routeList.length) {
			...
			// 加载路由
			return loadDynamicRoutes(routeList);
		}
	 ...
	 * @param r 路由对象
	 */
	const loadDynamicRoutes = (routers: RouterConfigRaw[], r = router) => {

		const resolveRoutes = processDynamicRoutes(processRouteMeta(routers));
	...
	const processRouteMeta = (routers: RouterConfigRaw[], basePath = "/") => {
		routers.forEach((route) => {
			...
			if (route.meta) {
				...
				const { routeUseI18n, isKeepAlive: isKeepAliveGlobal, isFull: isFullGlobal, routeUseTooltip } = serviceConfig.router;
