...
let goodsList: GoodsItem[] = [
	{
		...
		images: ["https://dummyimage.com/160x160/e6f4ff/1677ff&text=1", "https://dummyimage.com/160x160/fff1f0/c41d7f&text=2"],
		content: "轻薄夹克，适合春秋通勤。",
		is_sku: 1,
		limit_type: 1,
		limit_num: 2,
...
const shippingTemplateMap = () => new Map(shippingTemplates.filter((item) => item.is_effect === 1).map((item) => [item.id, item]));

const goodsCategoryNames = (cateIds: number[]) => cateIds.map((id) => categoryMap().get(id)?.name).filter(Boolean) as string[];
...
const validateCategory = (payload: CategoryPayload) => {
	...
	const parentExists = payload.parent_id === 0 || categories.some((item) => item.id === payload.parent_id && item.is_effect === 1);
	if (!parentExists) throw new Error("上级分类不存在");
	if (payload.id && getCategoryChildrenIds(payload.id).includes(payload.parent_id)) throw new Error("上级分类不能选择自己的子分类");

	const exists = categories.find((item) => item.is_effect === 1 && item.parent_id === payload.parent_id && item.name === name && item.id !== payload.id);
...
const normalizeSkuPriceList = (goodsId: number, skuPrices: GoodsPayload["sku_price_list"], normalizedSkus: GoodsSkuItem[]) => {
...
export const ShopService = {
	...
	async saveCategory(param: any) {
		return http.post<httpNs.Response>(param.id ? "/category/add" : "/category/edit", param);
	},
	...
	async saveShippingTemplate(payload: ShippingTemplatePayload) {
		...
		const time = now();
		if (payload.id) {
			shippingTemplates = shippingTemplates.map((item) => (item.id === payload.id ? { ...item, ...payload, update_time: time } : item));
	...
	async updateShippingTemplateStatus(id: number, status: 0 | 1) {
		shippingTemplates = shippingTemplates.map((item) => (item.id === id ? { ...item, status, update_time: now() } : item));
		return true;
	},
	...
	async deleteShippingTemplate(id: number) {
		...
		shippingTemplates = shippingTemplates.map((item) => (item.id === id ? { ...item, is_effect: 0, update_time: now() } : item));
		return true;
	},
	...
	async getGoodsList(query: GoodsQuery = {}) {
		...
		const list = goodsList
			.filter((item) => {
				...
				if (query.status !== "" && query.status !== undefined && item.status !== Number(query.status)) return false;
				if (query.cate_id !== "" && query.cate_id !== undefined && !item.cate_ids.includes(Number(query.cate_id))) return false;
				return true;
			})
			.map((item) => {
	...
	async saveGoods(payload: GoodsPayload) {
		...
		if (payload.id) {
			...
			goodsSkuPrices = goodsSkuPrices.map((item) => (item.goods_id === payload.id ? { ...item, is_effect: 0, update_time: time } : item));
	...
	async deleteGoods(id: number) {
		...
		goodsSkuPrices = goodsSkuPrices.map((item) => (item.goods_id === id ? { ...item, is_effect: 0, update_time: now() } : item));
		return true;
	},

	async changeGoodsStock(payload: StockPayload) {
		const row = goodsSkuPrices.find((item) => item.id === payload.goods_sku_price_id && item.goods_id === payload.goods_id && item.is_effect === 1);
